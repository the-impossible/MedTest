...
def notification(request):
    if request.user.is_authenticated:
        if not request.user.is_staff:
            ...
            filtered = (x for x in ScheduleTest.objects.filter(
                stud_id=stud.stud_id).order_by('-test_date') if not x.has_expired)
            all_test = ScheduleTest.objects.filter(
                stud_id=stud.stud_id).count()
            ...
            completed_test = StudentProfile.objects.filter(
                is_completed=True).count()
            incomplete_test = StudentProfile.objects.filter(
